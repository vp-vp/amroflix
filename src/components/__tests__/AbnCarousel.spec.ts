...
describe('ABN Carousel', () => {
    ...
    it('renders carousel card correctly', () => {
        ...
        expect(wrapper.find('[data-testid="carousel-card-1-subtitle"]').element.innerHTML).toEqual(showsList[0].name)
        expect(wrapper.find('[data-testid="carousel-card-1-img"]').element.getAttribute('aria-label')).toEqual(showsList[0].image.medium)
        expect(wrapper.find('[data-testid="carousel-card-1-img-placeholder"]').element.innerHTML).toEqual(showsList[0].name)
    })
    ...
    it('renders carousel card image placeholder', () => {
        ...
        expect(wrapper.find('[data-testid="carousel-card-2-subtitle"]').element.innerHTML).toEqual(showsList[1].name)
        expect(wrapper.find('[data-testid="carousel-card-2-img"]').element.getAttribute('aria-label')).toEqual(showsList[1].image.medium)
        expect(wrapper.find('[data-testid="carousel-card-2-img-placeholder"]').element.innerHTML).toEqual(showsList[1].name)
    })
})
